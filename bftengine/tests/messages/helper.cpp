...
bftEngine::impl::SigManager* createSigManager(size_t myId,
  ...
  for (const auto& e : publicKeysOfReplicas) {
    if (e.first == myId) continue;
    publickeys.push_back(e.second);
    publicKeysMapping.insert({e.first, j++});
  }

  // return createSigManager(myId, myPrivateKey, publickeys, publicKeysMapping);
  return SigManager::init(myId, myPrivateKey, publickeys, publicKeysMapping);
}
